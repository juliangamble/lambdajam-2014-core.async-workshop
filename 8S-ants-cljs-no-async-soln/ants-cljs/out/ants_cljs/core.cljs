...
;TODO: work out whether adding ant index increases perf
(ns ants-cljs.core
  (:require
    [goog.dom :as dom]
    [goog.events :as ev]
...
;note - replaced with defrecord

;(def log-flags #{"behave" "behave-ants" "update-ants" "move" "create-ant" "setup" "pprint"})
;(def log-flags #{"drop-food" "count" "render-ant"})
(def log-flags #{"ca" "cas"})
...
;world is a 2d vector of refs to cells (more performant? better with a map of coords?)
(def world 
     (apply vector 
            (map (fn [_] 
...
(defn pprint 
  "debugging technique for clojurescript - handles maps and vectors but not records"
  [cljs-obj]
...
;pre and postconditions
  ...
  ;TODO move into checker method
         (when (nil? oldp)
           (do
             (logj "move" "nil oldp in move")
             (throw (js/Error. "move - no oldp at oldloc"))
             (do-pause)))
         (when (nil? ant)
           (do
             (logj "update-ants" "nil antp in move")
             (throw (js/Error. "move - no ant at oldloc"))
             (do-pause)))
         (when (nil? p)
           (do
             (logj "move" "nil p in move")
             (throw (js/Error. "move - no p at newloc"))
             (do-pause)))
         (if (not (nil? (:ant @p)))
           (do
           ...
           (do
             (swap! p assoc :ant ant)
              (logj "ca" (str"writing ant movement from " coord " to " newloc " to render queue"))
             (when (not (nil? (:ant @p)))
               (do
     ...
     (if (:food ant)
       ...
       (cond 
        ...
        (let [ranks (merge-with + 
          ...
          ;feels like it should be turn then move...?
          ;TODO: should these be let?
...
(defn render-ant [ant ctx x y]
  (logj "cas" (str "is running: " is-running))
  (logj "cas" (str "rendering ant at: " x y))
  (let [[hx hy tx ty] ({0 [2 0 2 4] 
    ...
    (doto ctx
      ...
      (.stroke))
    (when (:food ant)
      (logj "render-ant" "ant info:" ant))
    (when (:id ant)
      (logj "render-ant" "ant has id: " ant))
    (when-not (:id ant)
      (logj "render-ant" "ant has no id: " ant))))
...
(defn draw-outside-box 
  "draw the boundary box around the world"
  [ctx]
  (doto ctx
    (.beginPath)
    (.rect 1 1 (* dim scale) (* dim scale)))
...
(defn draw-home-box 
  "draw the blue box around the home square"
  [ctx]
  (logj "cas" "draw-home-box")
   (doto ctx
    (.beginPath)
...
(defn render 
  ...
  ;TODO: core.async - add channel in here
  [ctx]
  (logj "cas" (str "is running: " is-running))
...
(def canvs 
  "get the object representing the context"
  (get-canvas "antcanvas"))

(def context 
  "map the graphics context to the js canvas"
  (.getContext canvs "2d"))

(defn behave-ants 
  "change the state of the ants 1 turn for each ant"
  []
  (go (while true   (if is-running
    (let [ant-coords (get-ant-coords)]
      (logj "move" (str "get-ants-coords: " ant-coords))
      (logj "count" (str "get-ant-food-count " (get-ant-food-count))) 
      (logj "cas" "running behave-ants")
      (dorun
       (for [coord ant-coords]
         (try
           (do
       ;TODO: remove this stuff? Just debugging? Extract as checker method?
       ; Change to runtime assertion in submethod?
             (let [p (place coord)
                   ant (:ant @p)]
               (when (nil? p)
                 (do
               ...
               (when (nil? ant)
                 (do
                   ...
                   (throw (js/Error. (str "behave-ants - no ant at this location" coord)))
                   (do-pause))))
             (behave coord))
           (catch js/Object e
             (.log js/console e)))))))
...
(defn send-off-animation 
  "integrate the old animation call into the js method"
  []
  (render context)) 
...
(defn run-loop 
  "the main game loop"
  []
  ;TODO: Wrap in try-catch that call pause - so that assertion errors can
  ;bubble up here
    (send-off-animation)
    (behave-ants)
    (evaporate))
